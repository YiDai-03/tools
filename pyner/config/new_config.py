...
configs = {
    ...
    'embedding_dir': path.sep.join([BASE_DIR, 'output/embedding']),# 词向量保存路�?

    'valid_size': 0.1, # valid数据集大�?
    'min_freq': 1,     # 最小词频，构建语料
    'num_classes': 12, # 类别个数 这里主要还有pad 0
    'max_length': 80,  # word文本平均长度,按照覆盖95%样本的标准，取截断长�?np.percentile(list,95.0)
    ...
    'n_gpus': [], # GPU个数,如果只写一个数字，则表示gpu标号�?开始，并且默认使用gpu:0作为controller,
